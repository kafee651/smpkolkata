...
class twitter_widget extends WP_Widget
{
	...
	{
		$widget_ops = array('classname' => 'box', 'description' => 'Show Twitter Widget Tweets on your website' );
...
?>
	<p>
		<label for="<?php echo $this->get_field_id('title'); ?>">
		  <?php esc_html_e('Widget Title:','theneeds');?>
	...
	<p>
	   <?php esc_html_e('Please visit this link first','theneeds');?>
		<a href="https://dev.twitter.com/apps/new">https://dev.twitter.com/apps/new</a> <?php _e('it will take to page where you can make twitter app that will help you to get some details that you need to activate your twitter widget.','theneeds');?>
		  
	</p>
	
	<p>
		<label for="<?php echo $this->get_field_id('consumer_key'); ?>">
		<?php esc_html_e('Consumer Key','theneeds');?>
		</label>
		<br/>
	...
	<p>
		<label for="<?php echo $this->get_field_id('consumer_secret'); ?>">
		 <?php esc_html_e('Consumer Secret Key','theneeds');?>
		</label> 
		<br/ >
	...
	<p>
		...
		<?php esc_html_e('User Token','theneeds');?>
		</label>
		<br/ >
	...
	<p>
		<label for="<?php echo $this->get_field_id('user_secret'); ?>">
		   <?php esc_html_e('User Secret Token','theneeds');?>
		</label>
		<br/ >
	...
	<p>
		...
		<?php esc_html_e('User Name (without twitter url for example: http://www.twitter.com/Envato write only Envato): ','theneeds');?>
		</label>
		<br/ >
	...
	<p>
	  <label for="<?php echo $this->get_field_id('num_of_tweets'); ?>">
		  <?php esc_html_e('Number of Tweets','theneeds');?>
	  </label>
	  <br/ >
            ...
            <ul>
				...
				if($tweets <> ''){	
					 foreach ($tweets as $tweet) { ?>
						<li>
							<p><?php echo mb_substr(esc_attr($tweet->text), 0 , 81);?>... <a href="http://twitter.com/<?php echo esc_attr($tweet->user->screen_name); ?>/status/<?php echo $tweet->id_str;?>"><?php esc_html_e("@","crunchpress"); echo esc_attr($tweet->user->name); ?></a></p>
							<i class="fa fa-twitter" aria-hidden="true"></i>
							<div class="holde"> <span class="name"><?php esc_html_e("@","crunchpress"); echo esc_attr($tweet->user->name); ?></span> <span class="time"><?php echo human_time_diff((strtotime($tweet->created_at)));esc_html_e(" ago","crunchpress")?></span> </div>
						</li>
					<?php 
